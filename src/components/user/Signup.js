...
import emailValidation from '../utils/validEmail'
...
const handleSignup =()=>{
    const myEmailValidation = emailValidation(email,password);
if(myEmailValidation!==null){
   setError(myEmailValidation)
   return 
}
...
createUserWithEmailAndPassword(auth, email, password)
 ...
 <h1 className='text-4xl'>Signup</h1>
  ...
  <button className="btn btn-active btn-neutral w-2/3 m-auto flex items-center gap-2 mb-4" onClick={handleSignup}>Signup</button>
  <p className='text-center text-blue-500'><Link to={"/signin"}>Already a user? SignIn now</Link></p>
  </div>
  </div>
  </>
  )
